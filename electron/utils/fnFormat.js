...
exports.CURRENT_SCHEMA_VERSION = exports.FN_MAGIC = void 0;
exports.serializeFn = serializeFn;
exports.deserializeFn = deserializeFn;
exports.writeFnFile = writeFnFile;
exports.readFnFile = readFnFile;
exports.getRawBinaryInfo = getRawBinaryInfo;
exports.validateFnFile = validateFnFile;
const fs_1 = __importDefault(require("fs"));
const path_1 = __importDefault(require("path"));
const crypto_1 = __importDefault(require("crypto"));
exports.FN_MAGIC = 'FN01'; // 4 bytes: 0x46, 0x4E, 0x30, 0x31
...
function serializeFn(content) {
    const magicBuf = Buffer.from(exports.FN_MAGIC, 'ascii'); // 4 bytes
    const headerMetaBuf = Buffer.alloc(4 + 36 + 8 + 8 + 4); // schemaVersion(4) + topicUid(36) + created(8) + updated(8) + sectionCount(4)
    headerMetaBuf.writeUInt32BE(content.header.schemaVersion || exports.CURRENT_SCHEMA_VERSION, 0);
 ...
 */
function deserializeFn(buffer) {
    if (buffer.length < 60 + 32) {
        throw new Error('Invalid .fn binary buffer: file size too small');
    }
    const magic = buffer.toString('ascii', 0, 4);
    if (magic !== exports.FN_MAGIC) {
        throw new Error(`Invalid .fn magic header: expected ${exports.FN_MAGIC}, got ${magic}`);
    }
    ...
    let document = { pages: [] };
    let assets = { images: [] };
    let semantic = { chunks: [], relationships: [] };
    let rag = {
            ...
            if (typeId === SECTION_TYPES.TOPIC_METADATA) {
                ...
                const deserializedEmbeddings = (parsedRag.embeddings || []).map((emb) => {
                    ...
                    if (emb.vectorBase64) {
                        ...
                        const floatArr = new Float32Array(floatBuf.buffer, floatBuf.byteOffset, floatBuf.byteLength / Float32Array.BYTES_PER_ELEMENT);
 ...
 */
async function writeFnFile(filePath, content) {
 ...
 */
async function readFnFile(filePath) {
 ...
 */
async function getRawBinaryInfo(filePath) {
    const buffer = await fs_1.default.promises.readFile(filePath);
    if (buffer.length < 60 + 32) {
 ...
 */
async function validateFnFile(filePath) {
    const errors = [];
    const warnings = [];
    let content = null;
    let rawInfo = null;
    try {
        ...
        if (rawInfo.magic !== exports.FN_MAGIC) {
            errors.push(`Invalid magic header: expected '${exports.FN_MAGIC}', got '${rawInfo.magic}'`);
        }
    ...
    for (const chunk of chunks) {
        ...
        if (!chunk.contentHash)
            warnings.push(`Chunk ${chunk.id} missing contentHash`);
    }
    // Validate Embeddings
